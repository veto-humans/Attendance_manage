...
function normalizeManagedGrade(value) {
...
function createUser(payload) {
  ...
  if (typeof headerMap['managedgrade'] === 'number') row[headerMap['managedgrade']] = normalizeManagedGrade(payload.managedGrade) || '';
  sheet.appendRow(row);
  ...
  return {
    ...
    data: {
      ...
      managedGrade: normalizeManagedGrade(payload.managedGrade) || ''
    }
  };
}
...
function getUsersByGrade(grade) {
  ...
  const teachers = userRows
    .filter((row) => {
      ...
      const rowManagedGrade = normalizeManagedGrade(row[managedGradeIndex]);
      ...
      return normalizedClassName.startsWith(normalizedGrade);
    })
    .map((row) => {
      ...
      return {
        ...
        role: 'teacher',
        managedGrade: typeof managedGradeIndex === 'number' ? normalizeManagedGrade(row[managedGradeIndex]) || '' : ''
      };
    });
